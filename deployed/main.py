...
def fetch_counts():
    ...
    if (len(dates) == 4):
        ...
        val = (("%\"date\": \"" + dates[0] + "\", \"title\": \"Saturday\"%"),
               ("%\"date\": \"" + dates[1] + "\", \"title\": \"Saturday\"%"),
               ("%\"date\": \"" + dates[2] + "\", \"title\": \"Saturday\"%"),
               ("%\"date\": \"" + dates[3] + "\", \"title\": \"Saturday\"%"))
    if (len(dates) == 5):
        check_string_2 = "SELECT SUM(ptolist LIKE %s) , SUM(ptolist LIKE %s) , SUM(ptolist LIKE %s) , SUM(ptolist LIKE %s) FROM pto_record;"
        val = (("%\"date\": \"" + dates[0] + "\", \"title\": \"Saturday\"%"),
               ("%\"date\": \"" + dates[1] + "\", \"title\": \"Saturday\"%"),
...
def post_sat():
    ...
    payload = request.json
    update_saturday(payload['pool_a_level_1'], payload['pool_a_level_2'], payload['pool_b_level_1'],
                    payload['pool_b_level_2'], payload['date'])
...
if __name__ == '__main__':
    print("Starting . . . ")
    app.run(host='10.28.143.233', port=8080, debug=True, threaded=True)
