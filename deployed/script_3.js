...
function getData(){
  ...
  //console.log('Fetching from server');
	...
	(async () => {
		const rawResponse = await fetch('http://10.28.143.233:8080/getAll', {
			method: 'GET',
			headers: {
...
function getUsers(){
  ...
  //console.log('Fetching from server');
	...
	(async () => {
		...
		const rawResponse2 = await fetch('http://10.28.143.233:8080/getUsers', {
			method: 'GET',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
...
function countEntries(data){
	var counts;
	(async () => {
		const rawResponse = await fetch('http://10.28.143.233:8080/getCount', {
			method: 'GET',
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
...
function viz(dates,counts){
	...
	(async () => {
		...
		for await (const num of capacity){
			...
			dataArray.push(counts[count]/8*100)
			++count
		}
